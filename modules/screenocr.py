...
TOPK = 3
CKPT_PATH   = r".\ScreenOCR\runs\champion_cls_v2\model_e20.pth"
LABELS_JSON = r".\ScreenOCR\runs\champion_cls_v2\labels.json"
IMG_SIZE = 128
...
def ocr_box_with_retry(ocr, bgr, rc, kind, lex, strict, pad_pct, min_score=MIN_OCR_SCORE, retries=RETRY_MAX, delay=RETRY_DELAY):
    ...
    for i in range(retries + 1):
        txt, s = ocr_box_with_retry(ocr, bgr, rc, kind, lex, strict, pad_pct)
        if s >= min_score or i == retries:
            return txt, s
        time.sleep(delay)

def ocr_nick_with_retry(ocr, crop, lex, strict, min_score=MIN_OCR_SCORE, retries=RETRY_MAX, delay=RETRY_DELAY):
    for i in range(retries + 1):
        cands = ocr.candidates(crop, allow=None, force_upper=False)
        txt, s = rerank_with_lexicon(cands, lex, strict=strict)
        if s >= min_score or i == retries:
            return txt, s
        time.sleep(delay)

def predict_topk_with_retry(clf, crop, topk, min_prob=MIN_CLS_PROB, retries=RETRY_MAX, delay=RETRY_DELAY):
    for i in range(retries + 1):
        topk_list = clf.predict_topk(crop, topk=topk)
        if topk_list:
            name, prob = topk_list[0]
        else:
            name, prob = "", 0.0
        if prob >= min_prob or i == retries:
            return topk_list, name, prob
        time.sleep(delay)
...
def ocr_box_with_retry(ocr, bgr, rc, kind, lex, strict, pad_pct):
